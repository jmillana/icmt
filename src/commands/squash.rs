...
pub fn squash_workflow(mut chat_completions: chatgpty::GptyCompletions, args: &SquashArgs) {
    ...
    let commit_logs = git::get_commits(base_branch).unwrap_or_else(|| {
        spinner.stop_and_persist(
            "✖".red().to_string().as_str(),
    ...
    log::info!("Commits: {:?}", commit_logs);
    let n_commits = commit_logs.len();
    // Generate the squash message
    let prompt = prompts::get_squash_user_prompt(commit_logs, &args.hint);
    ...
    spinner.stop_and_persist(
        ...
        "Successfully loaded commits.".green().to_string(),
    );
    if auto_accept
